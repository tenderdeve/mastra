...
import { CloudExporter, MastraObserveExporter } from './cloud';
...
describe('MastraObserveExporter', () => {
  ...
  describe('deprecated CloudExporter alias', () => {
    it('points at the same constructor as MastraObserveExporter', () => {
      expect(CloudExporter).toBe(MastraObserveExporter);
    });

    it('produces instances that are MastraObserveExporter', () => {
      const aliased = new CloudExporter({
        accessToken: testJWT,
        endpoint: 'http://localhost:3000',
      });
      expect(aliased).toBeInstanceOf(MastraObserveExporter);
    });
  });
});
