...
`workingMemory.version: 'vnext'` is deprecated and will be removed in a future major release. It has been superseded by [Observational Memory](/docs/memory/observational-memory). To retain working memory vnext behavior, [write a custom processor](#custom-incremental-updates).
...
Working memory vnext has been deprecated and superseded by [Observational Memory](/docs/memory/observational-memory). To retain working memory vnext behavior, disable built-in working memory and use a custom processor with your own storage, read logic, and update logic.

See [Retaining working memory vnext behavior with a custom processor](/guides/guide/incremental-working-memory) for a step-by-step guide.
