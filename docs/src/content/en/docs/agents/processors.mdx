...
Handles provider-specific history incompatibilities when agents reuse messages across model providers. It can rewrite the outbound LLM prompt before the provider call, or recover from known provider API errors and retry.
