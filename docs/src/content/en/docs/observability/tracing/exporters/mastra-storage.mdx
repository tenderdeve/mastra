    ...
    configs: {
      default: {
        ...
        exporters: [
          ...
          new MastraObserveExporter(), // Sends observability data to hosted Mastra Studio (requires MASTRA_CLOUD_ACCESS_TOKEN)
        ],
...
If you set the strategy to `'auto'`, the `MastraStorageExporter` automatically selects the optimal strategy for the storage provider. If you set the strategy to a mode that the storage provider doesn't support, you will get an error message.
...
- [MastraObserveExporter](/docs/observability/tracing/exporters/mastra-observe)
