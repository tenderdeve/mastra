---
title: "Mastra Observe exporter | Tracing | Observability"
description: "Send traces, logs, metrics, scores, and feedback to Mastra platform"
packages:
  - "@mastra/observability"
---

# Mastra Observe exporter

The `MastraObserveExporter` sends traces, logs, metrics, scores, and feedback to the Mastra platform. Use it to route observability data from any Mastra app to a hosted project in the Mastra platform.

:::note

`MastraObserveExporter` was previously called `CloudExporter`. The original `CloudExporter` class is still exported from `@mastra/observability` for backward compatibility, but it is deprecated. New code should use `MastraObserveExporter`.

:::

:::note[Self-hosted or standalone apps]

If you host your Mastra application on your own infrastructure (not on Mastra Platform), you still need a deployed Studio project to view traces, logs, and metrics. `MastraObserveExporter` sends data to a Studio project, so one must exist before you can use it.
...
- The exporter is available starting in `@mastra/observability@1.8.0`. In versions before the `MastraObserveExporter` rename, import it as `CloudExporter` instead — the constructor signature and environment variables are identical.
...
To connect `MastraObserveExporter`, create an access token, find the destination `projectId`, and add the exporter to your observability config.
...
Set both values in your environment so `MastraObserveExporter` can authenticate and route telemetry to the correct project:
...
When you pass a base origin, `MastraObserveExporter` derives the matching publish URLs for traces, logs, metrics, scores, and feedback automatically.

### 4. Enable `MastraObserveExporter`

The following example demonstrates how to add `MastraObserveExporter` to your observability config:

```ts title="src/mastra/index.ts"
import { Mastra } from '@mastra/core'
import { Observability, MastraObserveExporter } from '@mastra/observability'
  ...
  observability: new Observability({
    configs: {
      production: {
        ...
        exporters: [new MastraObserveExporter()],
      },
    },
  }),
})
...
Set `serviceName` on the observability config, not on `MastraObserveExporter` itself.
...
With `MASTRA_CLOUD_ACCESS_TOKEN` and `MASTRA_PROJECT_ID` set, `MastraObserveExporter` sends data to the Mastra platform project you configured. If you also set `MASTRA_CLOUD_TRACES_ENDPOINT`, it sends data to that collector instead.

:::note

Visit [MastraObserveExporter reference](/reference/observability/tracing/exporters/cloud-exporter) for the full list of configuration options.
...
`MastraObserveExporter` defaults to Mastra platform. If you want to send telemetry to a different collector, set `MASTRA_CLOUD_TRACES_ENDPOINT` in your environment or pass `endpoint` in code.
...
After you enable `MastraObserveExporter`, open your project in [Mastra Studio](https://projects.mastra.ai) to inspect the exported data.
...
MastraObserveExporter uses batching to optimize network usage. Events are buffered and sent in batches, reducing overhead while maintaining near real-time visibility.
...
- [MastraStorageExporter](/docs/observability/tracing/exporters/default)
