...
- Use `MastraObserveExporter` with `@mastra/observability@1.8.0` or later.
...
- Starting in `@mastra/observability@1.9.2`, `MastraObserveExporter` defaults to `https://observability.mastra.ai`, so `MASTRA_CLOUD_TRACES_ENDPOINT` is only required when you want to send telemetry to a different collector.
