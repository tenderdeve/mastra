...
- `MastraObserveExporter` is available starting in `@mastra/observability@1.12.0`. In `1.8.0` through `1.11.x` the same exporter ships only as `CloudExporter`; the constructor signature and environment variables are identical.
...
Visit [MastraObserveExporter reference](/reference/observability/tracing/exporters/mastra-observe-exporter) for the full list of configuration options.
