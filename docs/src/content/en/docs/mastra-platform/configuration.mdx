...
Each project has its own Studio URL and its own observability data. When using [`MastraObserveExporter`](/docs/observability/tracing/exporters/mastra-observe), set `MASTRA_PROJECT_ID` and `MASTRA_CLOUD_ACCESS_TOKEN` per environment so traces route to the matching Studio project.
