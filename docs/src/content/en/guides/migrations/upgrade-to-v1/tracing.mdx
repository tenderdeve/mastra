...
Complete this migration in the same change that bumps your Mastra packages, and verify traces appear in [Mastra Studio](/docs/studio/observability) before considering the upgrade complete. If you previously hosted on Mastra Cloud, also follow the [Mastra Cloud migration guide](/guides/migrations/mastra-cloud) — the new platform requires a new access token and a Studio project before `MastraObserveExporter` can route data to it.
:::

:::note[Exporter rename]

`MastraObserveExporter` (sends data to Mastra Observe) replaces the previous `CloudExporter`, and `MastraStorageExporter` (persists data to Mastra Storage) replaces the previous `DefaultExporter`. The original classes still ship from `@mastra/observability` and behave identically, but they are deprecated. New code should use `MastraObserveExporter` and `MastraStorageExporter`; existing imports of `CloudExporter`/`DefaultExporter` continue to work until they are removed in a future major version.
...
This configuration includes `MastraStorageExporter`, `MastraObserveExporter`, and `SensitiveDataFilter` processor. See the [observability tracing documentation](/docs/observability/tracing/overview) for full configuration options.
...
1. Use explicit `configs:` with `MastraStorageExporter`, `MastraObserveExporter`, and `SensitiveDataFilter`
