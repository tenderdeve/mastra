    ...
    properties: [
      {
        parameters: [
          ...
          },
          {
            ...
            type: 'WorkingMemory',
            description:
              "Configuration for working memory. Can be `{ enabled: boolean; template?: string; schema?: ZodObject<any> | JSONSchema7; scope?: 'thread' | 'resource'; version?: 'stable' | 'vnext' }` or `{ enabled: boolean }` to disable. The `version` option is deprecated. `version: 'vnext'` has been superseded by [Observational Memory](/reference/memory/observational-memory).",
            isOptional: true,
            defaultValue:
