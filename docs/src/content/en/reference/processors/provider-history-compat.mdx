...
<PropertiesTable
  ...
  {
    name: 'processLLMPrompt',
    type: '(args: ProcessLLMPromptArgs) => ProcessLLMPromptResult',
    description:
...
Preemptive rules run through `processLLMPrompt` after Mastra converts messages to the model prompt format and before the prompt is sent to the provider. These rewrites affect only the current provider call.
