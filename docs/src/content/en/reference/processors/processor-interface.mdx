...
│  │             ▼                                           │    │
│  │  ┌─────────────────────┐                                │    │
│  │  │  processLLMPrompt   │  ← Runs before provider call   │    │
│  │  └──────────┬──────────┘                                │    │
│  │             │                                           │    │
│  │             ▼                                           │    │
...
| `processLLMPrompt` | After LLM prompt conversion, before the provider call | Rewrite the outbound `LanguageModelV2Prompt` for the current call without persisting changes |
...
1. `processLLMPrompt` from inputProcessors (after prompt conversion, before the provider call)
1. LLM execution
...
Processes the final LLM prompt after Mastra converts the `MessageList` into `LanguageModelV2Prompt` and before the provider call. Use this method for transient, model-aware rewrites that should affect only the current outbound request.
...
<PropertiesTable
  content={[
  {
    ...
    description: 'The LLM prompt that will be sent to the provider for this call.',
    isOptional: false,
  },
  {
    name: 'model',
...
`processLLMPrompt` returns `LanguageModelV2Prompt | undefined | void`.
