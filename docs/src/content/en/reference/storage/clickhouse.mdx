...
`MastraStorageExporter` automatically selects the `insert-only` strategy when ClickHouse is the observability backend, which gives the highest write throughput. See [tracing strategies](/docs/observability/tracing/exporters/default#tracing-strategies) for details.
...
For the full strategy matrix and production guidance, see the [`MastraStorageExporter` reference](/docs/observability/tracing/exporters/default#storage-provider-support).
