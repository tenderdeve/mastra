...
The `@mastra/redis` package also exports `RedisServerCache`, a Redis-backed implementation of `MastraServerCache`. Use it as the Mastra instance's server cache for distributed caching of workflow stream buffers and processor LLM responses.
...
`RedisServerCache` supports per-key TTL. When a processor sets `cacheLLMResponse: true`, detection results are stored with per-entry TTL in Redis, so stale entries expire automatically.
...
<PropertiesTable
  content={[
  {
    name: 'options',
    type: 'Options',
    description: 'Configuration for the Redis server cache',
    isOptional: false,
    properties: [
      {
        type: 'Options',
        parameters: [
          ...
          },
          {
            ...
            description: 'Pre-configured redis client. Use this for Sentinel, Cluster, or shared connections.',
            isOptional: true,
          },
          {
            name: 'keyPrefix',
            type: 'string',
            description: 'Prefix for all cache keys. Useful for namespacing in shared Redis instances.',
            isOptional: true,
