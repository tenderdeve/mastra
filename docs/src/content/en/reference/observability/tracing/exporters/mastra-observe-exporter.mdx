...
Wire format for exported spans. The exporter spreads the original `AnyExportedSpan` (so the original field names are preserved) and layers a small set of platform-friendly aliases on top:
