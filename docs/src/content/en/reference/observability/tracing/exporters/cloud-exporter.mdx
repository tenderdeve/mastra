...
description: API reference for the deprecated CloudExporter (replaced by MastraObserveExporter)
packages:
...
**Added in:** `@mastra/observability@1.8.0`. **Deprecated in `1.12.0`** in favor of [`MastraObserveExporter`](/reference/observability/tracing/exporters/mastra-observe-exporter).

:::warning[Deprecated]

`CloudExporter` is retained for backward compatibility and will be removed in a future major version. Use [`MastraObserveExporter`](/reference/observability/tracing/exporters/mastra-observe-exporter) for new projects. The two classes share the same constructor, environment variables, and runtime behavior; `CloudExporter` keeps its original `mastra-cloud-observability-exporter` exporter `name` and `CLOUD_EXPORTER_*` error IDs so monitoring rules built against it keep working.
