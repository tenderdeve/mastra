---
title: "Reference: MastraObserveExporter | Observability"
description: API reference for the MastraObserveExporter (formerly CloudExporter)
packages:
...
**Added in:** `@mastra/observability@1.8.0` (originally as `CloudExporter`)

Sends tracing spans, logs, metrics, scores, and feedback to the Mastra platform for online visualization and monitoring.

:::note

`MastraObserveExporter` was previously called `CloudExporter`. The original `CloudExporter` class is still exported from `@mastra/observability` so existing imports keep working, but it is deprecated and will be removed in a future major version. New code should use `MastraObserveExporter`.
...
new MastraObserveExporter(config?: MastraObserveExporterConfig)
```
...
<PropertiesTable
  ...
  {
    name: 'config',
    type: 'MastraObserveExporterConfig',
...
## `MastraObserveExporterConfig`

```typescript
interface MastraObserveExporterConfig extends BaseExporterConfig {
...
- `MASTRA_CLOUD_ACCESS_TOKEN` - Authentication token for `MastraObserveExporter` requests
...
```typescript prettier:false
readonly name = 'mastra-observe-exporter';
```
...
Every `LogEvent` passed to this handler is buffered and exported to the logs endpoint derived from the configured base endpoint. Unlike tracing, there is no additional event-status filtering at the `MastraObserveExporter` level. If the exporter is disabled, this method becomes a no-op.
...
Every `MetricEvent` passed to this handler is buffered and exported to the metrics endpoint derived from the configured base endpoint. There is no additional filtering by metric subtype or status inside `MastraObserveExporter`; the exporter forwards every metric event it receives unless it is disabled.
...
Every `FeedbackEvent` passed to this handler is buffered and exported to the feedback endpoint derived from the configured base endpoint. There is no feedback-type filtering inside `MastraObserveExporter`; all feedback events received here are forwarded unless the exporter is disabled.
...
Errors raised by `MastraObserveExporter` use the `MASTRA_OBSERVE_EXPORTER_*` `id` prefix. The deprecated `CloudExporter` continues to emit `CLOUD_EXPORTER_*` `id`s.
...
## `MastraObserveSpanRecord`

Internal format for exported spans:

```typescript
interface MastraObserveSpanRecord {
  traceId: string
  spanId: string
...
const exporter = new MastraObserveExporter()

// Explicit configuration
const customExporter = new MastraObserveExporter({
...
- [MastraStorageExporter](/reference/observability/tracing/exporters/default-exporter): Storage persistence
