...
wraps `ExportedFeedback` in `feedback`. For Mastra Observe exporter behavior for these
callbacks, see [MastraObserveExporter](/reference/observability/tracing/exporters/mastra-observe-exporter).
