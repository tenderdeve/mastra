...
Metrics are routed through an internal event bus to the `MastraStorageExporter`, which batches and flushes them to storage.
...
- **`MastraStorageExporter` is present**: Other exporters (Datadog, Langfuse, etc.) don't persist metrics to Mastra storage. `MastraStorageExporter` is required for the Studio dashboard.
