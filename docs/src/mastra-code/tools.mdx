...
- If a task omits an `id`, Mastra assigns one and returns it in the tool result.
- Only one task should be `in_progress` at a time.

### `task_update`

Update one tracked task by ID. The agent uses this to change a task's `content`, `status`, or `activeForm` without rewriting the full task list.

### `task_complete`

Mark one tracked task completed by ID. The agent uses this immediately after finishing a tracked task.

### `task_check`

Check the completion status of the current task list. The agent uses this before finishing to verify all tasks are complete. The result also includes the structured task list snapshot with task IDs.
